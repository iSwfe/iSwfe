...
function operate(opt)
{
	...
	default:
		//如果最后一个是"[+-*/]",则替换成现在的+-*/
		if ( boxVal.charAt(boxLen-1).match("[\\+\\-\\*\\/]") )
			delBoxLast();
		//寻找前面的运算符,有则立即计算
		boxVal = document.getElementById(rstBoxID).value;	//更新下刚才可能删除LastOne的boxValue
		if ( boxVal.substring(1).search("[\\+\\-\\*\\/]") >=0 )	//从[1]开始截到最后,再做[+-*/]的匹配,避免"-321"就满足的情况
			calc();
...
function getNumArr(orgBoxStr)	//originalBoxString:原始的Box框字符串
{
	...
	else if ( -1!=newBoxStr.lastIndexOf("-") && newBoxStr.search("[\\+\\-\\*\\/]")!=newBoxStr.lastIndexOf("-") )
	{
		num2Char = "-";
